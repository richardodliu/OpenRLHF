...
## 总评
本章已修复多处历史问题（包括 seq-mask 计数分情况、loss 方向、显式补充部分阈值条件）。当前剩余问题集中在两类：`thm:prefix-tighter` 的陈述-证明一致性，以及 `thm:prefix-adaptive` 中 sample-level 公式与阈值界限处理不一致。
...
3. P1: 对齐 Part (a) 的 statement 与 proof，并修正非对称阈值下的界限表达。
