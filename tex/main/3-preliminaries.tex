...
\end{equation*}
where we use $d_s^{\pi}(c_s) = d_s^{\pi'}(c_s)$ since the two policies share the same prefix distribution up to position $s$ (the divergence occurs at position $s$, not before). Since $d_s^{\pi}(c_s) > 0$ and $\pi(v^*|c_s) \neq \pi'(v^*|c_s)$, we have $d_{s+1}^{\pi}(c_s, v^*) \neq d_{s+1}^{\pi'}(c_s, v^*)$, and therefore $\|d_{s+1}^{\pi} - d_{s+1}^{\pi'}\|_{\mathrm{TV}} > 0$.

\emph{Inductive step ($t > s+1$).} Fix any sequence of tokens $(y_{s+1}^*, \ldots, y_{t-1}^*)$ such that the prefix $c_t^* = (c_s, v^*, y_{s+1}^*, \ldots, y_{t-1}^*)$ is reachable under both policies. By the support overlap condition, all transition probabilities along this path are positive under both $\pi$ and $\pi'$. The visitation probability factorizes as:
\begin{equation*}
    d_t^{\pi}(c_t^*) = d_{s+1}^{\pi}(c_s, v^*) \cdot \prod_{s'=s+1}^{t-1} \pi(y_{s'}^*|c_{s'}^*).
\end{equation*}
Since $d_{s+1}^{\pi}(c_s, v^*) \neq d_{s+1}^{\pi'}(c_s, v^*)$ (from the base case) and $\prod_{s'=s+1}^{t-1} \pi(y_{s'}^*|c_{s'}^*) > 0$, the product $d_t^{\pi}(c_t^*) \neq d_t^{\pi'}(c_t^*)$. This exhibits a specific context $c_t^*$ at which the two visitation distributions differ, so $\|d_t^{\pi} - d_t^{\pi'}\|_{\mathrm{TV}} > 0$. The exact magnitude can be bounded via the KL chain rule and Pinsker's inequality (see \Cref{thm:prefix-adaptive}).
\end{proof}
