...
\begin{tabular}{lllll}
\toprule
\textbf{Method} & \textbf{IS Statistic} & \textbf{Gate / Truncation} & \textbf{Granularity} & \textbf{Causal IS} \\
\midrule
TIS & $\rho_t$ & $w_t^{\mathrm{tok}}=\mathrm{clip}(\rho_t,1-\epslow,1+\epshigh)$ & Token & No \\
ICEPOP & $\rho_t$ & $M_t^{\mathrm{tok}}=\mathbb{I}[\rho_t \in [1-\epslow,1+\epshigh]]$ & Token & No \\
TRM & $\rho_t$ & $M^{\mathrm{TRM}}(x,y)=\mathbb{I}\!\big[\max_t\,\Dkltok(c_t;\piroll,\pitheta)\le \delta_{\mathrm{TRM}}\big]$ & Sequence & No \\
GSPO & $\rhoseq$ & $w^{\mathrm{seq}}=\mathrm{clip}(\rhoseq,1-\epslow,1+\epshigh)$ & Sequence & No \\
\bottomrule
\end{tabular}
\end{table}
