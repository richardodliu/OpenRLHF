...
\par
\noindent\textbf{Prefix-level Causal Trust Region masking.} Our prefix mask $M_t^{\mathrm{prefix}}$ implements a causal, per-position gate
constructed from prefix ratio statistics. Compared with TRM's sequence-level gate $M(x,y)$, it preserves positional granularity by masking
...
\begin{tabular}{lllll}
\toprule
\textbf{Method} & \textbf{IS Ratio} & \textbf{Mask / Clip} & \textbf{Granularity} & \textbf{Causal} \\
\midrule
TIS & $\rho_t$ & $\mathrm{clip}(\rho_t,\lambda,\Lambda)$ & Token & No \\
ICEPOP & $\rho_t$ & $\mathbb{I}[\rho_t \in [\lambda, \Lambda]]$ & Token & No \\
TRM & $\rho_t$ & $M(x,y)$ (max-KL) & Sequence & No \\
GSPO & $\rhoseq$ & $\mathrm{clip}(\rhoseq,\lambda,\Lambda)$ & Sequence & No \\
\bottomrule
\end{tabular}
\end{table}
