...
\label{def:max-div}
For the policy pair being analyzed (e.g., $(\pitheta,\piroll)$ in trust-region bounds or $(\piold,\piroll)$ in mismatch analysis), define:
\begin{equation}
    \epsilon := \Dtvtokmax := \max_{t,\,c_t} \Dtvtok(c_t), \qquad
    \delta := \Dkltokmax := \max_{t,\,c_t} \Dkltok(c_t), \qquad
    \Dbar_t := \E_{c_t \sim d_t^{\piroll}}[\Dtvtok(c_t)].
\end{equation}
\end{definition}
...
\label{asm:support}
For all contexts $c_t$ and tokens $y_t$ in the vocabulary, $\piroll(y_t|c_t) > 0 \implies \piold(y_t|c_t) > 0$ and $\pitheta(y_t|c_t) > 0$. This ensures that the rollout mismatch ratio $w_t = \piold(y_t|c_t)/\piroll(y_t|c_t)$ and the surrogate ratio $\rho_t = \pitheta(y_t|c_t)/\piroll(y_t|c_t)$ are well-defined, and that KL terms with $\piroll$ in the first argument are finite.
\end{assumption}
...
\end{equation}
where $\Dbar_t = \E_{c_t \sim d_t^{\piroll}}[\Dtvtok(c_t)]$ is the expected per-position TV divergence (between $\pitheta$ and $\piroll$ in the TRM setting), $\epsilon = \Dtvtokmax$, and $\delta = \Dkltokmax$. This bound operates at per-position granularity and preserves the causal structure of the error accumulation---properties that motivate our prefix cumulative IS correction.
