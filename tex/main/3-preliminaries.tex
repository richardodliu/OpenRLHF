...
\textbf{IS ratios.} The per-token importance sampling (IS) ratio and its log-ratio are
\begin{align}
    \IStoken{t}
...
\noindent\textbf{TRM ratio notation.} Following trust-region masking style analyses~\citep{li2025trm}, we also write
\begin{equation}
    \rho_t := \IStoken{t},
    \qquad
    \log \rho_t = r_t.
...
ratio) must be anchored to the original behavior distribution that generated the data. Accordingly, we define $\IStoken{t}$ in
...
\midrule
TIS & $\IStoken{t}$ clamped to $[\lambda, \Lambda]$ & Token & No \\
ICEPOP & $\IStoken{t} \cdot \mathbb{I}[\IStoken{t} \in [\lambda, \Lambda]]$ & Token & No \\
TRM & $M^{\mathrm{TRM}}(x,y) \cdot \IStoken{t}$ & Sequence & No \\
GSPO & $\ISseq$ & Sequence & No \\
\bottomrule
\end{tabular}
\end{table}

\noindent Here $\ISseq$ is the sequence-level geometric-mean ratio,
\begin{equation}
    \ISseq := \exp(\bar{r}),
    \qquad
    \bar{r} := \frac{\sum_t r_t m_t}{\sum_t m_t},
\end{equation}
and $m_t$ is the action mask. TRM (Trust Region Masking, \citet{li2025trm}) uses a binary \emph{sequence-level} mask
$M^{\mathrm{TRM}}(x,y)$ to discard trajectories that violate the trust region. A standard (max-based) criterion is
\begin{equation}
    M^{\mathrm{TRM}}(x,y)
