...
For IS masking, we use the acceptance interval $[1-\epslow,1+\epshigh]=[0.5,5.0]$ unless otherwise stated (i.e., $\epslow=0.5$ and $\epshigh=4.0$), and synthetic log-ratio patterns with $T=32$ to directly instantiate the three regimes in \Cref{thm:prefix-tighter}.
...
\begin{table}[ht]
\centering
\caption{Masking behavior under synthetic deviation patterns ($T=32$, $[1-\epslow,1+\epshigh]=[0.5,5.0]$). We report the number of masked tokens and the masked position range.}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{lccc}
\toprule
...
while prefix masking remains localized in position. For example, in the late-deviation pattern above, using $[1-\epslow,1+\epshigh]=[0.8,2.0]$
...
\begin{itemize}
    ...
    \item Thresholds: default $[1-\epslow,1+\epshigh]=[0.5,5.0]$; sensitivity example $[0.8,2.0]$.
