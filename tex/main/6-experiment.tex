...
\textbf{Baseline self-correlation.} We measure $\mathrm{corr}(r_i, b_i)$ between each sample reward $r_i$ and its baseline $b_i$. A correlated baseline can couple a sample with its own control variate, breaking the clean second-moment factorization used in RLOO analyses.
...
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{Mean baseline / global norm} & \textbf{RLOO / adaptive norm} \\
\midrule
$\mathrm{corr}(r_i, b_i)$ & $0.351$ & $-0.005$ \\
...
\noindent Prefix masking rejects a contiguous suffix once the prefix average log-ratio crosses the threshold, preserving positional information that is lost by sequence-level masking, and masking more off-policy tokens than token-level masking in the early-deviation regime (as predicted by \Cref{thm:prefix-tighter}).
...
\begin{itemize}
    ...
    \item Reward distribution: mixture with $p=0.15$ and Gaussian noise $\sigma=0.10$.
    \item Token lengths: success $T_i \approx 256$, failure $T_i \approx 512$ with small integer jitter.
    \item Thresholds: default $[\lambda,\Lambda]=[0.5,5.0]$; sensitivity example $[0.8,2.0]$.
    \item Code version: git commit \texttt{a330f8a}.
\end{itemize}
