...
Adaptive normalization is not related to PPO's asymmetric clipping, which modifies the importance ratio. Our asymmetry comes from
sign-dependent rescaling of advantages under the token-level expansion and the empirical constraints in \Cref{def:adaptive-norm}.
...
\]
Therefore the closed-form scales $(\alpha,\beta)$ in \eqref{eq:alpha-beta} depend not only on reward magnitudes but also on the ratio of
