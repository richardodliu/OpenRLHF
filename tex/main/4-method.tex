...
\label{def:prefix-is}
For a response trajectory $y=(y_1,\ldots,y_T)$ with per-token log-ratios $r_t$ defined in \eqref{eq:log-ratio}, define the prefix
cumulative sums
\begin{align}
    L_t &:= \sum_{s=1}^{t} r_s, \label{eq:prefix-cumsum} \\
    \bar{r}_t &:= \frac{1}{t}L_t, \qquad \rhoprefix{t} := \exp(\bar{r}_t), \label{eq:prefix-is}
\end{align}
for $t\in\{1,\ldots,T\}$. The statistic $\rhoprefix{t}$ is the geometric mean of per-token importance ratios over the length-$t$ prefix.
\end{definition}

\begin{definition}[Prefix Causal Trust Region Mask (Proxy)]
...
\textbf{Causality.} $\rhoprefix{t}$ depends only on $\{r_s\}_{s \le t}$, respecting the autoregressive order.
...
Fix thresholds $0<\lambda\le 1\le \Lambda$, and define $\bar{r}_t$ as in \eqref{eq:prefix-is}. Suppose
$\bar{r}_t\in[\log\lambda,\log\Lambda]$ and
$r_{t+1}\in[\log\lambda,\log\Lambda]$, then $\bar{r}_{t+1}\in[\log\lambda,\log\Lambda]$.
\end{lemma}

\begin{proof}
By definition, $L_{t+1} = L_t + r_{t+1}$, hence
\begin{equation}
    \bar{r}_{t+1}
    = \frac{1}{t+1}L_{t+1}
    = \frac{t}{t+1}\,\bar{r}_t + \frac{1}{t+1}\,r_{t+1}.
\end{equation}
...
\begin{enumerate}
    ...
    satisfying
    \begin{equation}
        t < \frac{r_1+\varepsilon}{\log\Lambda+\varepsilon},
        \label{eq:early-upper-sufficient}
    \end{equation}
    we have $M_t^{\mathrm{prefix}}=0$. Symmetrically, if $r_1<\log\lambda$ then for every position $t$ satisfying
    \begin{equation}
        t < \frac{-r_1+\varepsilon}{|\log\lambda|+\varepsilon},
...
only $t=1$. For the prefix statistic, write $L_t=r_1+\sum_{s=2}^{t}r_s$ and use the bounds
\begin{align}
    \sum_{s=2}^{t}r_s &\ge -(t-1)\varepsilon,
    \\
    \sum_{s=2}^{t}r_s &\le (t-1)\varepsilon,
\end{align}
...
comparison follows from $t^*\le T$.
...
When all $r_s$ share the same sign but do not all exceed the corresponding threshold, the running average $L_t/t$ need not be
...
\begin{equation}
    ...
    \iff \frac{1}{t}\sum_{s=1}^{t}r_s \in [\log\lambda, \log\Lambda].
...
\begin{equation}
    \frac{1}{t}\sum_{s=1}^{t} \log\rho_s \in [\log\lambda, \log\Lambda].
\end{equation}
...
\begin{proof}
Follows directly from \Cref{def:prefix-is} and Eq.~\eqref{eq:prefix-mask}: $M_t^{\mathrm{prefix}} = 1$ iff $\rhoprefix{t} \in [\lambda, \Lambda]$ iff $L_t/t \in [\log\lambda, \log\Lambda]$. Since $L_t = \sum_{s=1}^{t} r_s$ and $r_s=\log\rho_s$, the claim follows.
\end{proof}
