...
\section{REINFORCE Pro: Prefix Trust-Region Masking}
...
\begin{equation}
    \rho_t := \IStoken{t} = \frac{\pitheta(y_t \mid c_t)}{\piroll(y_t \mid c_t)},
\end{equation}
as defined in \eqref{eq:ratio}. Throughout this section, we restrict attention to masking strategies constructed solely from
$(\rho_t)_{t=1}^T$ (equivalently, the log-ratios $r_t=\log\rho_t$ in \eqref{eq:log-ratio}), and we do not introduce additional
intermediate policies.

\paragraph{A TRM-style view.}
...
\begin{equation}
    L_{\mathrm{masked}}^{\mathrm{TRM}}(\pitheta)
    := \E_{\piroll}\!\left[
        M^{\mathrm{TRM}}(x,y)\cdot A(x,y)\cdot \sum_{t=1}^{T}\rho_t
    \right].
    \label{eq:trm-masked-surrogate}
\end{equation}
Our method can be viewed as a prefix-level refinement of this mechanism: instead of a single sequence gate, we compute a per-position
mask $M_t^{\mathrm{prefix}}$ from a ratio-based \emph{prefix} statistic and multiply it into the token-level loss, thereby retaining
per-position granularity while respecting the autoregressive order. For comparison, a prefix-level masked surrogate can be written as
\begin{equation}
    L_{\mathrm{masked}}^{\mathrm{prefix}}(\pitheta)
    := \E_{\piroll}\!\left[
...
\Cref{sec:combined-loss} for the exact objective. Importantly, our prefix criterion is a tractable sample-level \emph{proxy} for
trust-region style filtering and is not the exact TRM-Max criterion based on $\max_t \DKL(c_t)$.
...
\begin{equation}
    \IStoken{1} \notin [\lambda,\Lambda],
    \qquad
    \IStoken{t} \in [\lambda,\Lambda]\ \text{ for all }\ t\in\{2,\ldots,T\}.
    \label{eq:token-early-deviation}
\end{equation}
Then the token-level ICEPOP mask,
\begin{equation}
    M_t^{\mathrm{token}} := \mathbb{I}\!\big[\IStoken{t} \in [\lambda,\Lambda]\big],
\end{equation}
...
\subsection{Prefix Trust-Region Proxy: Definition and Properties}
\label{sec:prefix-is}
...
\begin{align}
    ...
    \bar{r}_t &= \frac{L_t}{P_t}, \qquad \ISprefix{t} = \exp(\bar{r}_t), \label{eq:prefix-is}
\end{align}
where $\bar{r}_t$ is defined only for $t$ such that $P_t\ge 1$ (i.e., the prefix contains at least one active action token). The
statistic $\ISprefix{t}$ is the geometric mean of per-token IS ratios over active positions up to $t$.
\end{definition}

\begin{definition}[Prefix TRM-Style Masking (Proxy)]
\label{def:prefix-trm-proxy}
Given thresholds $[\lambda, \Lambda]$, define the per-position prefix mask
\begin{equation}
    M_t^{\mathrm{prefix}} := \mathbb{I}\!\big[\ISprefix{t} \in [\lambda, \Lambda]\big].
...
\textbf{Causality.} $\ISprefix{t}$ depends only on $\{r_s\}_{s \le t}$, respecting the autoregressive order.
...
\end{equation}
and $M_t^{\mathrm{seq}} := \mathbb{I}[\ISseq \in [\lambda,\Lambda]]$ for all $t$.
Assume policy support overlap (\Cref{asm:support}). Then:

\begin{enumerate}
    \item[(a)] \textbf{Early deviation: the prefix mask rejects a (possibly longer) prefix.} Assume $\IStoken{1} \notin [\lambda, \Lambda]$ and
    $|r_t| \le \varepsilon$ for all $t>1$, where $0 \le \varepsilon \le \min(\log\Lambda, |\log\lambda|)$. Then token-level ICEPOP masks
    only position $t=1$, while the prefix mask rejects at least position $t=1$. Moreover, if $r_1>\log\Lambda$ then for every position $t$
...
only $t=1$. For the prefix statistic, write $L_t=r_1+\sum_{s=2}^{t}r_s m_s$ and use the bounds
\begin{align}
...
We relate prefix masking to the per-position, causal structure that appears in trust-region style error bounds for long-horizon
...
Thresholding $\ISprefix{t}$ is equivalent to a per-position, sample-level constraint on the prefix average log-ratio:
\begin{equation}
    \ISprefix{t} \in [\lambda, \Lambda]
...
Under prefix TRM-style masking with threshold $[\lambda, \Lambda]$, for all accepted tokens ($M_t^{\mathrm{prefix}} = 1$), the sample-level cumulative log-ratio satisfies:
\begin{equation}
    \frac{1}{P_t}\sum_{s=1}^{t} \log\rho_s \cdot m_s \in [\log\lambda, \log\Lambda].
\end{equation}
...
\begin{proof}
Follows directly from \Cref{def:prefix-is} and Eq.~\eqref{eq:prefix-mask}: $M_t^{\mathrm{prefix}} = 1$ iff $\ISprefix{t} \in [\lambda, \Lambda]$ iff $L_t/P_t \in [\log\lambda, \log\Lambda]$. Since $L_t = \sum_{s\le t} r_s m_s$ and $r_s=\log\rho_s$, the claim follows.
\end{proof}
...
TRM-style mask from \Cref{def:prefix-trm-proxy},
