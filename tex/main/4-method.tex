...
\begin{equation}
    \alpha S^+ = \beta S^-,
    \qquad
    \alpha^2 Q^+ + \beta^2 Q^- = |\mathcal{P}| + |\mathcal{N}|.
\end{equation}
\end{definition}

\begin{proposition}[Closed-Form Solution]
\label{prop:alpha-beta}
Assume $\mathcal{P} \neq \emptyset$ and $\mathcal{N} \neq \emptyset$. The unique solution satisfying $\alpha S^+ = \beta S^-$ and
$\alpha^2 Q^+ + \beta^2 Q^- = |\mathcal{P}| + |\mathcal{N}|$ is:
\begin{equation}
    \alpha = \sqrt{\frac{|\mathcal{P}| + |\mathcal{N}|}{Q^+ + \left(\frac{S^+}{S^-}\right)^{\!2} Q^-}},
    \qquad
...
\textbf{Step 1 (zero empirical mean).} The constraint $\alpha S^+ = \beta S^-$ implies
\begin{equation}
    \beta = \alpha \cdot \frac{S^+}{S^-}.
...
\textbf{Step 2 (unit empirical second moment).} Substituting \eqref{eq:beta-from-mean} into
$\alpha^2 Q^+ + \beta^2 Q^- = |\mathcal{P}| + |\mathcal{N}|$ yields
\begin{align}
\alpha^2
&= \frac{|\mathcal{P}| + |\mathcal{N}|}{Q^+ + \left(\frac{S^+}{S^-}\right)^{2} Q^-}.
\end{align}
The denominator is strictly positive because $Q^+>0$, $Q^->0$, and $|\mathcal{P}| + |\mathcal{N}|>0$ under the same assumption. Taking
the positive square root gives the stated $\alpha>0$, and then \eqref{eq:beta-from-mean} yields the stated $\beta$.
The mean constraint uniquely specifies the ratio $\beta/\alpha$, and the second-moment constraint uniquely
...
\paragraph{Connection to asymmetric clipping.}
Adaptive normalization is a soft analog of PPO's asymmetric clipping. PPO clips the importance ratio differently depending on the advantage
sign; adaptive normalization scales the advantage magnitude differently. Both control the effective step size asymmetrically, but
normalization acts through the advantage rather than the ratio, avoiding the gradient-leakage failure mode of ratio clipping under mismatch
(see \Cref{app:ppo-clip-leak}).
...
\begin{proof}
By definition, \eqref{eq:token-early-deviation} implies $M_1^{\mathrm{tok}}=0$ and $M_t^{\mathrm{tok}}=1$ for all $t\ge2$.
...
\begin{equation}
    M_t^{\mathrm{prefix}} := \mathbb{I}\!\big[\rhoprefix{t} \in [1-\epslow, 1+\epshigh]\big].
    \label{eq:prefix-mask}
\end{equation}
\end{definition}

Appendix~\ref{app:mask-variants} collects several compatible variants (Variants~B--E) that modify the prefix gate while retaining the same
...
Fix thresholds $\epslow\in(0,1)$ and $\epshigh>0$, and define $\rhoprefix{t}$ as in \eqref{eq:prefix-is}. If
\begin{equation}
...
Let $M^{\mathrm{prefix}}_t$, $M^{\mathrm{tok}}_t$, and $M^{\mathrm{seq}}_t$ denote the prefix-level mask
...
\begin{enumerate}
    ...
    \end{equation}
    we have $M_t^{\mathrm{prefix}}=0$. Symmetrically, if $\log\rho_1<-\tau_-$ then for every position $t$ satisfying
    \begin{equation}
        t < \frac{-\log\rho_1+\varepsilon}{\tau_- + \varepsilon},
    ...
    $\log\rho_s < -\tau_-$ for all $s$ (lower violation), then $M_t^{\mathrm{prefix}} = 0$ and $M_t^{\mathrm{seq}} = 0$ for all $t$,
...
Under prefix Causal Trust Region masking with threshold $[1-\epslow, 1+\epshigh]$, for all accepted tokens ($M_t^{\mathrm{prefix}} = 1$), the sample-level cumulative log-ratio satisfies:
\begin{equation}
    \log\rhoprefix{t}
    = \frac{1}{t}\sum_{s=1}^{t} \log\rho_s
...
\begin{proof}
By \Cref{def:prefix-trm-proxy}, $M_t^{\mathrm{prefix}} = 1$ implies $\rhoprefix{t}\in[1-\epslow,1+\epshigh]$. Taking logarithms yields
$\log\rhoprefix{t}\in[\log(1-\epslow),\log(1+\epshigh)]$. The identity $\log\rhoprefix{t} = \frac{1}{t}\sum_{s=1}^{t}\log\rho_s$ follows
