...
We now establish the theoretical link between prefix cumulative IS and the Adaptive bound from the trust region framework~\citep{li2025trm}. \emph{Important scope note:} The following theorem highlights structural correspondences between the two frameworks. Parts~(a) and~(b) are structural observations. Part~(c) states an expectation identity that holds over the full rollout distribution $\piroll$, and Eq.~\eqref{eq:threshold-kl} is an equivalence that defines the sample-level filtering criterion. This connection is used as a practical proxy: bounding the sample-level prefix log-ratio does not by itself imply a bound on population-level KL/TV (either under the full distribution or under the filtered/conditional distribution) without additional assumptions. Importantly, this section concerns only the $\piold/\piroll$ mismatch; relating it to $\pitheta$ requires separate assumptions on the PPO update and is discussed in \Cref{sec:unified}.

\begin{theorem}[Prefix IS as a Sample-Level Approximation of the Adaptive Bound]
\label{thm:prefix-adaptive}
Under Assumptions~\ref{asm:support}--\ref{asm:finite-length}, with threshold $0 < \lambda \le 1 \le \Lambda$, the Adaptive bound (\Cref{eq:adaptive-bound}) and prefix cumulative IS share three structural correspondences. Part~(c) connects the cumulative sample log-ratio to a joint KL term \emph{in expectation} via the KL chain rule, while thresholding $\mathrm{prefix\_is}(t)$ is equivalent to imposing a per-position, sample-level constraint on the prefix average log-ratio.
...
\begin{enumerate}
    ...
    \item[(b)] \textbf{Causal accumulation.} The Adaptive bound's factor $\Dbar_t = \E_{c_t \sim d_t^{\piroll}}[\Dtvtok(c_t)]$ captures the expected per-position TV divergence (the advantage factor), while the context shift factor $\|d_t^{\piold} - d_t^{\piroll}\|_{\mathrm{TV}}$ depends on the cumulative mismatch from positions $1$ to $t-1$. Prefix IS computes the prefix statistic $\mathrm{prefix\_is}(t) = \exp(L_t/P_t)$, which aggregates per-token log-ratios causally (only positions $s \le t$ contribute) and supports per-position filtering based on the prefix average log-ratio. This mirrors the per-position, causal structure of the Adaptive bound and motivates viewing prefix masking as a practical proxy for trust-region style filtering. (This section focuses on the $\piold/\piroll$ mismatch; relating to $\pitheta$ requires separate assumptions; see \Cref{sec:unified}.)
    ...
    \end{equation}
    Note that $L_t = \sum_{s=1}^t \ell_s m_s$ is a \emph{sample-level} quantity, while Eq.~\eqref{eq:cumsum-kl} is an expectation identity over the full rollout distribution. Eq.~\eqref{eq:threshold-kl} characterizes the per-trajectory acceptance condition used by prefix masking. Separately, Pinsker's inequality~\citep{pinsker1964information} relates the population-level KL in Eq.~\eqref{eq:cumsum-kl} to a TV distance:
    \begin{equation}
        \|d_{t+1}^{\piold} - d_{t+1}^{\piroll}\|_{\mathrm{TV}} \le \sqrt{\frac{\DKL(d_{t+1}^{\piroll} \| d_{t+1}^{\piold})}{2}}.
        \label{eq:context-shift-prefix}
    \end{equation}
    This inequality holds for the full distributions and does not depend on the filtering step. The sample-level constraint in Eq.~\eqref{eq:threshold-kl} should therefore be interpreted as a practical proxy for trust-region style filtering rather than a guaranteed population-level divergence bound without additional assumptions.
\end{enumerate}
\end{theorem}
...
Eq.~\eqref{eq:threshold-kl} follows directly from the definition $\mathrm{prefix\_is}(t) = \exp(L_t/P_t)$ and the mask rule $M_t^{\mathrm{prefix}} = \mathbb{I}[\mathrm{prefix\_is}(t) \in [\lambda, \Lambda]]$. Finally, Eq.~\eqref{eq:context-shift-prefix} is Pinsker's inequality applied to the pair of joint distributions $(d_{t+1}^{\piroll}, d_{t+1}^{\piold})$.
\end{proof}
