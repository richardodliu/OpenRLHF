...
For context, consider the trajectory-level surrogate $\tilde{\mathcal{L}}_{\piroll}(\pitheta)=\E_{\piroll}[A\cdot \sum_{t=1}^{T}\rho_t]$
from \Cref{def:impl-surrogate}. During a policy update, the behavior policy $\piroll$ is treated as fixed and rewards/baselines are
detached. Therefore, for a parameterized policy $\pi_\theta$ (denoted $\pitheta$), the gradient takes the score-function form
\begin{equation*}
    ...
    \right],
\end{equation*}
which follows from $\nabla_\theta \rho_t = \rho_t \nabla_\theta \log \pitheta(y_t\mid c_t)$ and is standard in policy-gradient
analyses (see, e.g., \citet{qi2026rethinkingtrustregionllm}).
\end{remark}
