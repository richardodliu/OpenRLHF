...
For a sequence of length $T$ where $|\ell_t| \le \varepsilon$ for all $t \neq t^*$ and $|\ell_{t^*}| = L$, the sequence-level geometric mean satisfies:
\begin{equation}
...
\begin{proof}
Under the stated conditions, $|\sum_{t \neq t^*} \ell_t| \le (T-1)\varepsilon$. Thus $\frac{1}{T}\sum_t \ell_t = \frac{L + O((T-1)\varepsilon)}{T} = \frac{L}{T} + O(\varepsilon)$. For $\varepsilon \to 0$, the geometric mean converges to $\exp(L/T)$, which approaches $1$ as $T \to \infty$ regardless of $L$.
\end{proof}
...
\noindent\textbf{Notation.} To avoid overloading symbols, we reserve $(\epsilon,\delta)$ for the divergence quantities in \Cref{sec:preliminaries}. In the deviation-pattern analysis below, we use $\varepsilon$ to denote a uniform bound on the per-token log-ratios $\ell_t$.
...
\begin{enumerate}
    ...
    $|\ell_t| \le \varepsilon$ for all $t>1$, where $0 \le \varepsilon \le \min(\log\Lambda, |\log\lambda|)$. Then token-level ICEPOP masks
    ...
    \begin{equation}
        P_t < \frac{\ell_1+\varepsilon}{\log\Lambda+\varepsilon},
...
\textbf{Part (a).} The $\varepsilon$-bounded condition implies $\ell_t \in [\log\lambda, \log\Lambda]$ for all $t>1$, hence ICEPOP masks
only $t=1$. For prefix IS, the bounds $\sum_{s=2}^{t}\ell_s m_s \ge -(P_t-1)\varepsilon$ and $\sum_{s=2}^{t}\ell_s m_s \le (P_t-1)\varepsilon$
