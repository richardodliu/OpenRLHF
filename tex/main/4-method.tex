...
We adopt the standard \emph{two-policy} view of PPO-style LLM-RL: samples are generated by a fixed behavior policy $\piold$, and the
training policy $\pitheta$ is updated from $\piold$ by gradient steps. This matches the trust-region setup in
\citet{li2025trm}, where $\piold$ plays the role of the rollout policy.

The per-token importance ratio between these two policies is
\begin{equation}
    \rho_t = \frac{\pitheta(y_t \mid c_t)}{\piold(y_t \mid c_t)},
\end{equation}
...
contexts $(c_t)_{t\ge2}$ are sampled under the behavior distribution induced by $\piold$, which differs from the context distribution that
...
On the other hand, since $\rho_1 \neq 1$, we have $\pitheta(y_1\mid c_1) \neq \piold(y_1\mid c_1)$, hence the conditional token
distributions $\pitheta(\cdot\mid c_1)$ and $\piold(\cdot\mid c_1)$ are not equal. Therefore $\Dtvtok(c_1;\piold,\pitheta)>0$.
Applying the context-shift propagation lemma (\Cref{lem:coupling}) with $(\pi,\pi')=(\piold,\pitheta)$ and $s=1$ yields
\begin{equation}
    \|d_t^{\piold} - d_t^{\pitheta}\|_{\mathrm{TV}} > 0
    \qquad
    \text{for all } t \ge 2.
\end{equation}
...
LLM-RL~\citep{li2025trm}. In our notation, these bounds analyze the policy pair $(\piold,\pitheta)$.
...
\begin{equation}
    \E_{\piold}\!\Big[\sum_{s=1}^{t} r_s\Big]
    = -\sum_{s=1}^{t} \E_{c_s \sim d_s^{\piold}}\!\big[\DKL(\piold(\cdot|c_s) \| \pitheta(\cdot|c_s))\big]
    = -\DKL(d_{t+1}^{\piold} \| d_{t+1}^{\pitheta}),
    \label{eq:cumsum-kl}
\end{equation}
...
\begin{proof}
We start from the definition $r_s = \log \pitheta(y_s\mid c_s) - \log \piold(y_s\mid c_s)$ in \eqref{eq:log-ratio}. Conditioning on
$c_s$ and taking expectation over $y_s\sim \piold(\cdot\mid c_s)$ gives
\begin{align}
    \E_{\piold}\!\big[r_s \mid c_s\big]
    &= \sum_{v} \piold(v\mid c_s)\,\log\frac{\pitheta(v\mid c_s)}{\piold(v\mid c_s)}
    \\
    &= - \sum_{v} \piold(v\mid c_s)\,\log\frac{\piold(v\mid c_s)}{\pitheta(v\mid c_s)}
    \\
    &= -\DKL\!\big(\piold(\cdot\mid c_s)\,\|\,\pitheta(\cdot\mid c_s)\big).
\end{align}
Taking expectation over $c_s\sim d_s^{\piold}$ and summing from $s=1$ to $t$ yields the first equality in \eqref{eq:cumsum-kl}.
...
\begin{equation}
    \DKL\!\big(d_{t+1}^{\piold}\,\|\,d_{t+1}^{\pitheta}\big)
    = \sum_{s=1}^{t}\E_{c_s \sim d_s^{\piold}}\!\Big[\DKL\!\big(\piold(\cdot\mid c_s)\,\|\,\pitheta(\cdot\mid c_s)\big)\Big].
\end{equation}
...
\begin{equation}
    \|d_{t+1}^{\pitheta} - d_{t+1}^{\piold}\|_{\mathrm{TV}}
    \le \sqrt{\frac{\DKL(d_{t+1}^{\piold} \| d_{t+1}^{\pitheta})}{2}}.
...
\begin{equation}
    \frac{1}{P_t}\sum_{s=1}^{t} \big(\log \pitheta(y_s|c_s) - \log \piold(y_s|c_s)\big) m_s \in [\log\lambda, \log\Lambda].
\end{equation}
...
We define the loss as the negative surrogate objective, $\mathcal{L} = -L_{\piold}(\pitheta)$, so that gradient descent on
