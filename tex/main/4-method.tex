...
\begin{align}
    ...
    \bar{r}_t &= \frac{L_t}{P_t}, \qquad \rhoprefix{t} = \exp(\bar{r}_t), \label{eq:prefix-is}
\end{align}
where $\bar{r}_t$ is defined only for $t$ such that $P_t\ge 1$ (i.e., the prefix contains at least one active action token). The
statistic $\rhoprefix{t}$ is the geometric mean of per-token importance ratios over active positions up to $t$.
\end{definition}
...
\begin{proof}
Follows directly from \Cref{def:prefix-is} and Eq.~\eqref{eq:prefix-mask}: $M_t^{\mathrm{prefix}} = 1$ iff $\rhoprefix{t} \in [\lambda, \Lambda]$ iff $L_t/P_t \in [\log\lambda, \log\Lambda]$. Since $L_t = \sum_{s\le t} r_s m_s$ and $r_s=\log\rho_s$, the claim follows.
\end{proof}
