...
\begin{enumerate}
    ...
    \item[(c)] \textbf{Monotone deviation: interpolation between token and sequence granularity.} When all per-token log-ratios have the same sign and each strictly exceeds the corresponding threshold---i.e., $\ell_s > \log\Lambda$ for all $s$ (upper violation), or $\ell_s < \log\lambda$ for all $s$ (lower violation)---the prefix cumulative mean $|L_t/P_t|$ is non-decreasing and remains strictly beyond the threshold for all $t$. In this regime:
    \begin{itemize}
