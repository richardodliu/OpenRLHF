...
\begin{equation}
    \rhoprefix{t}
    := \exp\!\left(\frac{1}{t}\sum_{s=1}^{t} r_s\right)
...
\begin{align}
    \log\rhoprefix{t+1}
    &= \frac{1}{t+1}\sum_{s=1}^{t+1} r_s
    \nonumber\\
    &= \frac{t}{t+1}\left(\frac{1}{t}\sum_{s=1}^{t} r_s\right) + \frac{1}{t+1}r_{t+1}
    \nonumber\\
...
When all $r_s$ share the same sign but do not all exceed the corresponding threshold, the prefix log-statistic $\log\rhoprefix{t}$ need not be
...
directly from \eqref{eq:prefix-is} and $r_s=\log\rho_s$.
\end{proof}
