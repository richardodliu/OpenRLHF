...
\begin{enumerate}
    \item[(a)] \textbf{Early deviation detection.} If $w_1 \notin [\lambda, \Lambda]$ and $|\ell_t| \le \epsilon$ for $t > 1$ with $0 \le \epsilon \le \min(\log\Lambda, |\log\lambda|)$, then prefix IS masks all positions $t \le \lfloor(|\ell_1| - \epsilon)/\max(|\log\lambda|, \log\Lambda)\rfloor$:
    \begin{equation}
        \big|\{t : M_t^{\mathrm{prefix}} = 0\}\big| \ge \big|\{t : M_t^{\mathrm{token}} = 0\}\big|.
    \end{equation}
    Prefix IS masks position 1 and a contiguous block of subsequent positions whose length depends on the magnitude of $\ell_1$ relative to the threshold and the perturbation $\epsilon$, while token-level IS masks only position 1.

    \item[(b)] \textbf{Late deviation efficiency.} If $|\ell_t| \le \epsilon$ for $t < t^*$ and $w_{t^*} \notin [\lambda, \Lambda]$ with $t^*$ close to $T$, then:
    \begin{equation}
    ...
    \item[(c)] \textbf{Monotone deviation: interpolation between token and sequence granularity.} When all per-token log-ratios have the same sign and each individually exceeds the corresponding threshold---i.e., $\ell_s \ge \log\Lambda$ for all $s$ (upper violation), or $\ell_s \le \log\lambda$ for all $s$ (lower violation)---the prefix cumulative mean $|L_t/P_t|$ is non-decreasing and remains beyond the threshold for all $t$. In this regime:
    \begin{itemize}
        ...
        \item The number of prefix-masked tokens satisfies $|\{t : M_t^{\mathrm{prefix}} = 0\}| \le |\{t : M_t^{\mathrm{seq}} = 0\}|$, since sequence-level rejection masks all $T$ tokens while prefix IS masks only the suffix from $t_0$ onward.
    \end{itemize}
...
\begin{enumerate}
    ...
    \item[(b)] \textbf{Causal accumulation.} The Adaptive bound's factor $\Dbar_t = \E_{c_t \sim d_t^{\piroll}}[\Dtvtok(c_t)]$ captures the expected per-position TV divergence (the advantage factor), while the context shift factor $\|d_t^{\pitheta} - d_t^{\piroll}\|_{\mathrm{TV}}$ depends on the cumulative divergence from positions $1$ to $t-1$. The prefix IS $\mathrm{prefix\_is}(t) = \exp(L_t/P_t)$ helps control this cumulative context shift on retained samples via the KL chain rule, providing a sample-level proxy for the context shift factor in the error decomposition.
    ...
    \begin{equation}
        \mathrm{prefix\_is}(t) \in [\lambda, \Lambda] \iff \frac{1}{P_t}\sum_{s=1}^{t}\ell_s \in [\log\lambda, \log\Lambda].
    ...
    \end{equation}
    The sample-level constraint $|L_t| \le t \cdot |\log\Lambda|$ does not directly imply a bound on the population-level KL; rather, it serves as a practical filter that excludes high-divergence trajectories, indirectly controlling the effective KL on the retained samples.
\end{enumerate}
\end{theorem}
...
\begin{equation}
    \frac{1}{P_t}\sum_{s=1}^{t} \big(\log \piold(y_s|c_s) - \log \piroll(y_s|c_s)\big) \in [\log\lambda, \log\Lambda].
\end{equation}
