...
\end{equation}
Assume there exists $t^*\in\{1,\ldots,T\}$ such that $\log\rho_{t^*} = L$ and $|\log\rho_t|\le \varepsilon$ for all $t\neq t^*$. Then
\begin{equation}
    \big|\log\rhoseq - L/T\big| \le \frac{T-1}{T}\,\varepsilon \le \varepsilon.
    \label{eq:seq-mean-dilution}
\end{equation}
In particular, in the pure single-spike case $\log\rho_{t^*}=L$ and $\log\rho_t=0$ for $t\neq t^*$, one has $\log\rhoseq=L/T$, so
$\rhoseq=\exp(L/T)\to 1$ as $T\to\infty$ for any fixed $L$.
\end{proposition}
