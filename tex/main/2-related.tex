...
sequence-level IS coefficient (the geometric-mean ratio $\ISseq$) and applies it uniformly across the sequence.
\par
...
mechanism: instead of a single sequence gate $M(x,y)$, we compute a per-position mask $M_t^{\mathrm{prefix}}$ from a ratio-based prefix
