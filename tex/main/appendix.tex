...
\textbf{Part (a).} Assume $w_1 \notin [\lambda,\Lambda]$ and $|\ell_t| \le \varepsilon$ for all $t>1$, with
$0 \le \varepsilon \le \min(\tau_+,\tau_-)$. Then for every $t>1$,
\[
    \ell_t \in [-\varepsilon,\varepsilon] \subseteq [-\tau_-,\tau_+],
\]
...
\]
If $\ell_1>\tau_+$, then $\ell_s m_s \ge -\varepsilon m_s$ for all $s\ge2$, so
\[
    L_t \ge \ell_1 - \varepsilon(P_t-1).
\]
Therefore, if $P_t < (\ell_1+\varepsilon)/(\tau_+ + \varepsilon)$ (the condition in \Cref{eq:early-upper-sufficient}), then
\[
    \frac{L_t}{P_t} \ge \frac{\ell_1-\varepsilon(P_t-1)}{P_t} > \tau_+,
\]
so $M_t^{\mathrm{prefix}}=0$. The lower-violation case $\ell_1<-\tau_-$ is symmetric: since $\ell_s m_s \le \varepsilon m_s$ for $s\ge2$,
we have $L_t \le \ell_1 + \varepsilon(P_t-1)$, and the condition in \Cref{eq:early-lower-sufficient} implies $L_t/P_t < -\tau_-$, hence
...
\textbf{Part (b).} Assume there exists $t^*\in\{1,\ldots,T\}$ such that $|\ell_t|\le \varepsilon$ for all $t\neq t^*$, with
$0 \le \varepsilon \le \min(\tau_+,\tau_-)$. For any $t<t^*$, all active log-ratios in the prefix satisfy $\ell_s \in [-\tau_-,\tau_+]$.
