...
\section{Leave-One-Out Baseline Properties (Summary)}
...
\begin{remark}[Properties of the Leave-One-Out Baseline (prior work)]
...
\begin{remark}[Optimality of $\gamma = 1.0$ (standard)]
When the extrinsic reward is sparse (assigned only at the final token), $\gamma = 1.0$ is the unique discount factor that preserves
unbiasedness of the REINFORCE gradient estimator for the undiscounted objective $J(\pitheta) = \E[R(x,y)]$; see
\citet{williams1992simple} for background.
\end{remark}
