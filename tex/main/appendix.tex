...
\begin{itemize}
    ...
    \item \textbf{Divergence proxies and lower bounds.} \Cref{app:dppo-proxies} collects several supporting identities and coarse-graining
    inequalities adapted from divergence-based trust-region analyses (e.g., \citet{qi2026rethinkingtrustregionllm}). These results connect
    sampled-token ratio statistics to distributional divergences and provide proof entry points for
    \Cref{lem:tv-ratio-identity,lem:coarse-grain-lower,lem:seq-tv-sum}.

    \item \textbf{Divergence approximation gaps.} \Cref{app:dppo-div-gap} bounds the slack introduced by TV coarse-graining and records
    equality conditions for KL coarse-graining, providing proof entry points for \Cref{rem:tv-gap,rem:kl-equality}.

    \item \textbf{Finite-horizon trust-region bounds.} \Cref{app:dppo-llm-tr} proves the finite-horizon performance difference identity and
    the associated remainder bounds recorded in \Cref{rem:fh-tr-bound}, adapted from \citet{qi2026rethinkingtrustregionllm}.

    \item \textbf{Surrogate gradient forms.} \Cref{app:grad-impl-surrogate} derives the gradient of the implementation surrogate
    $\tilde{\mathcal{L}}_{\piroll}$, providing a proof entry point for \Cref{rem:grad-impl-surrogate}.
...
\section{Divergence Proxies and Coarse-Grained Lower Bounds}
\label[appsec]{app:dppo-proxies}

This section provides proofs for the supporting statements in \Cref{sec:div-proxies}. The results are standard in divergence-based
trust-region analyses for the LLM regime (e.g., \citet{qi2026rethinkingtrustregionllm}) and are rewritten here in the notation of this
paper for completeness.

\subsection{Proof of \texorpdfstring{\Cref{lem:tv-ratio-identity}}{Lemma}}

\begin{proof}
Fix a context $c_t$. By definition of total variation distance,
\begin{align*}
    \Dtvtok(c_t;\piroll,\pitheta)
    &=
    \DTV\big(\piroll(\cdot|c_t)\,\|\,\pitheta(\cdot|c_t)\big)
    \\
    &=
    \frac{1}{2}\sum_{v} \big|\piroll(v|c_t) - \pitheta(v|c_t)\big|.
\end{align*}
For each token $v$ with $\piroll(v|c_t)>0$, define the ratio $\rho(v) := \pitheta(v|c_t)/\piroll(v|c_t)$. Then
\begin{align*}
    \big|\piroll(v|c_t) - \pitheta(v|c_t)\big|
    =
    \piroll(v|c_t)\,\big|1-\rho(v)\big|.
\end{align*}
Substituting this expression into the previous display yields
\begin{align*}
    \Dtvtok(c_t;\piroll,\pitheta)
    &=
    \frac{1}{2}\sum_{v} \piroll(v|c_t)\,\left|1-\frac{\pitheta(v|c_t)}{\piroll(v|c_t)}\right|
    \\
    &=
    \frac{1}{2}\E_{y_t \sim \piroll(\cdot|c_t)}\!\left[\big|1-\rho_t\big|\right]
    =
    \frac{1}{2}\E_{y_t \sim \piroll(\cdot|c_t)}\!\left[\big|\rho_t-1\big|\right],
\end{align*}
where $\rho_t = \pitheta(y_t|c_t)/\piroll(y_t|c_t)$ is the per-token importance ratio along the sampled trajectory. This completes the
proof.
\end{proof}

\subsection{Proof of \texorpdfstring{\Cref{lem:coarse-grain-lower} and \Cref{cor:binary-tv-lower}}{Lemma and Corollary}}

\begin{proof}
Fix a context $c_t$ and let $\mathcal{C}=\{C_1,\ldots,C_m\}$ be a partition of the vocabulary. For brevity, write
$p(v) := \pi(v|c_t)$ and $q(v):=\pi'(v|c_t)$, and define the coarse-grained masses
$p(C_j) := \sum_{v\in C_j} p(v)$ and $q(C_j) := \sum_{v\in C_j} q(v)$.

\textbf{TV lower bound.} By the triangle inequality applied within each cell,
\begin{align*}
    \sum_{v\in C_j} |p(v)-q(v)|
    &\ge
    \left|\sum_{v\in C_j} (p(v)-q(v))\right|
    =
    |p(C_j)-q(C_j)|.
\end{align*}
Summing over cells and dividing by $2$ gives
\begin{align*}
    \DTV(p\|q)
    =
    \frac{1}{2}\sum_{v} |p(v)-q(v)|
    &=
    \frac{1}{2}\sum_{j=1}^{m}\sum_{v\in C_j} |p(v)-q(v)|
    \\
    &\ge
    \frac{1}{2}\sum_{j=1}^{m} |p(C_j)-q(C_j)|
    =
    \DTV(p^{\mathcal{C}}\|q^{\mathcal{C}}),
\end{align*}
which is the claimed TV coarse-graining inequality.

\textbf{KL lower bound.} Assume $p$ is absolutely continuous with respect to $q$. For each cell $C_j$, the log-sum inequality implies
\begin{align*}
    \sum_{v\in C_j} p(v)\log\frac{p(v)}{q(v)}
    \ge
    \left(\sum_{v\in C_j} p(v)\right)\log\frac{\sum_{v\in C_j} p(v)}{\sum_{v\in C_j} q(v)}
    =
    p(C_j)\log\frac{p(C_j)}{q(C_j)}.
\end{align*}
...
\begin{align*}
    ...
    &=
    \sum_{j=1}^{m}\sum_{v\in C_j} p(v)\log\frac{p(v)}{q(v)}
    \\
    &\ge
    \sum_{j=1}^{m} p(C_j)\log\frac{p(C_j)}{q(C_j)}
    =
    \DKL(p^{\mathcal{C}}\|q^{\mathcal{C}}),
\end{align*}
which is the claimed KL coarse-graining inequality.

\textbf{Binary corollary.} Now specialize to the binary partition
$\mathcal{C}=\{\{y_t\},\mathcal{V}\setminus\{y_t\}\}$. The coarse-grained TV is
\begin{align*}
    \DTV(p^{\mathcal{C}}\|q^{\mathcal{C}})
    &=
    \frac{1}{2}\Big(
    ...
    |p(y_t)-q(y_t)|.
\end{align*}
Applying the TV lower bound with this partition gives $\DTV(p\|q)\ge |p(y_t)-q(y_t)|$. Finally, when $p(y_t)>0$,
\begin{align*}
    |p(y_t)-q(y_t)|
    =
    p(y_t)\left|1-\frac{q(y_t)}{p(y_t)}\right|
    =
    \piroll(y_t|c_t)\,\big|1-\rho_t\big|
    =
    \piroll(y_t|c_t)\,\big|\rho_t-1\big|.
\end{align*}
This proves \Cref{cor:binary-tv-lower}.
\end{proof}

\subsection{Proof of \texorpdfstring{\Cref{lem:seq-tv-sum}}{Lemma}}

\begin{proof}
Fix a prompt $x$ and define the sequence distributions
\begin{equation*}
    P(y) := P^{\piroll}(y\mid x) = \prod_{t=1}^{T}\piroll(y_t\mid c_t),
    \qquad
    Q(y) := P^{\pitheta}(y\mid x) = \prod_{t=1}^{T}\pitheta(y_t\mid c_t),
\end{equation*}
where $c_t=(x,y_{<t})$ is the context induced by the prefix of $y$.
By definition of TV distance,
\begin{equation*}
    2\,\DTV(P\|Q)
    =
    \sum_{y} \left| \prod_{t=1}^{T}\piroll(y_t\mid c_t) - \prod_{t=1}^{T}\pitheta(y_t\mid c_t) \right|.
\end{equation*}
We use the telescoping product identity
\begin{equation*}
    \prod_{t=1}^{T} a_t - \prod_{t=1}^{T} b_t
    =
    \sum_{t=1}^{T}\left(\prod_{k=1}^{t-1} a_k\right)(a_t-b_t)\left(\prod_{j=t+1}^{T} b_j\right),
\end{equation*}
applied to $a_t=\piroll(y_t\mid c_t)$ and $b_t=\pitheta(y_t\mid c_t)$. By the triangle inequality,
\begin{align*}
    2\,\DTV(P\|Q)
    &\le
    \sum_{y}\sum_{t=1}^{T}
    \left(\prod_{k=1}^{t-1}\piroll(y_k\mid c_k)\right)
    \big|\piroll(y_t\mid c_t)-\pitheta(y_t\mid c_t)\big|
    \left(\prod_{j=t+1}^{T}\pitheta(y_j\mid c_j)\right).
\end{align*}
Swap the order of summation and marginalize out the suffix $(y_{t+1},\ldots,y_T)$: for each fixed prefix $(y_1,\ldots,y_t)$, one has
\begin{equation*}
    \sum_{y_{t+1:T}} \prod_{j=t+1}^{T}\pitheta(y_j\mid c_j) = 1.
\end{equation*}
Therefore,
\begin{align*}
    2\,\DTV(P\|Q)
    &\le
    \sum_{t=1}^{T}\sum_{y_{\le t}}
    \left(\prod_{k=1}^{t-1}\piroll(y_k\mid c_k)\right)
    \big|\piroll(y_t\mid c_t)-\pitheta(y_t\mid c_t)\big|
    \\
    &=
    \sum_{t=1}^{T}\sum_{y_{< t}}
    \left(\prod_{k=1}^{t-1}\piroll(y_k\mid c_k)\right)
    \sum_{y_t}\big|\piroll(y_t\mid c_t)-\pitheta(y_t\mid c_t)\big|.
\end{align*}
Recognizing the inner sum as $2\,\DTV(\piroll(\cdot|c_t)\,\|\,\pitheta(\cdot|c_t))$ and the outer sum as an expectation over
$c_t\sim d_t^{\piroll}$ gives
\begin{equation*}
    2\,\DTV(P\|Q)
    \le
    2\sum_{t=1}^{T} \E_{c_t \sim d_t^{\piroll}}\!\big[\Dtvtok(c_t;\piroll,\pitheta)\big].
\end{equation*}
Dividing by $2$ completes the proof.
\end{proof}

\section{Divergence Approximation Gaps and Equality Conditions}
\label[appsec]{app:dppo-div-gap}

This section complements the coarse-graining lower bounds in \Cref{lem:coarse-grain-lower} by quantifying the slack introduced by TV
coarse-graining and recording the equality condition for KL coarse-graining. The arguments are standard (e.g.,
\citet{qi2026rethinkingtrustregionllm}).

\subsection{Proof of \texorpdfstring{\Cref{rem:tv-gap}}{Remark}}

\begin{proof}
Fix a context $c_t$ and write $p(v):=\pi(v|c_t)$ and $q(v):=\pi'(v|c_t)$. For each cell $C_j$, define the slack
\begin{equation*}
    s_j(c_t)
    :=
    \sum_{v\in C_j} \big|p(v)-q(v)\big|
    -
    \left|\sum_{v\in C_j}\big(p(v)-q(v)\big)\right|.
\end{equation*}
By the triangle inequality, $s_j(c_t)\ge 0$. Moreover,
\begin{align*}
    \DTV\big(\pi(\cdot|c_t)\,\|\,\pi'(\cdot|c_t)\big)
    &=
    \frac{1}{2}\sum_{j=1}^{m}\sum_{v\in C_j} \big|p(v)-q(v)\big|,
    \\
    \DTV\big(\pi^{\mathcal{C}}(\cdot|c_t)\,\|\,{\pi'}^{\mathcal{C}}(\cdot|c_t)\big)
    &=
    \frac{1}{2}\sum_{j=1}^{m} \left|p(C_j)-q(C_j)\right|
    =
    \frac{1}{2}\sum_{j=1}^{m} \left|\sum_{v\in C_j}\big(p(v)-q(v)\big)\right|.
\end{align*}
Subtracting the two displays yields the gap identity
\begin{equation*}
    \DTV\big(\pi(\cdot|c_t)\,\|\,\pi'(\cdot|c_t)\big)
    -
    \DTV\big(\pi^{\mathcal{C}}(\cdot|c_t)\,\|\,{\pi'}^{\mathcal{C}}(\cdot|c_t)\big)
...
\end{equation*}
Finally, since $\left|\sum_{v\in C_j}(p(v)-q(v))\right|\ge 0$, one has
\begin{equation*}
    s_j(c_t)
    \le
    \sum_{v\in C_j}\big|p(v)-q(v)\big|
    ...
    p(C_j)+q(C_j),
\end{equation*}
where the second inequality uses $|a-b|\le a+b$ for $a,b\ge 0$. This proves the claimed slack bound.
\end{proof}

\subsection{Proof of \texorpdfstring{\Cref{rem:kl-equality}}{Remark}}

\begin{proof}
Fix a context $c_t$ and recall the absolute continuity assumption in \Cref{lem:coarse-grain-lower}, so $q(v)>0$ whenever $p(v)>0$. In the
proof of \Cref{lem:coarse-grain-lower}, we apply the log-sum inequality within each cell $C_j$:
\begin{equation*}
    \sum_{v\in C_j} p(v)\log\frac{p(v)}{q(v)}
    \ge
    \left(\sum_{v\in C_j}p(v)\right)\log\frac{\sum_{v\in C_j}p(v)}{\sum_{v\in C_j}q(v)}
    ...
    p(C_j)\log\frac{p(C_j)}{q(C_j)}.
\end{equation*}
The log-sum inequality holds with equality if and only if the ratio $p(v)/q(v)$ is constant over all $v\in C_j$ with $p(v)>0$. Since the
coarse-grained KL lower bound is obtained by summing these inequalities over $j$, equality holds globally if and only if the condition
holds for every cell.
\end{proof}

\section{Finite-Horizon Trust Region Bounds}
\label[appsec]{app:dppo-llm-tr}

This section provides proofs for the finite-horizon surrogate identity and remainder bounds in \Cref{rem:fh-tr-bound}. The results are
adapted from divergence-based analyses for the LLM regime (e.g., \citet{qi2026rethinkingtrustregionllm}) and are stated in the notation of
this paper.

\subsection{Proof of the Identity in \texorpdfstring{\Cref{rem:fh-tr-bound}}{Remark}}

\begin{proof}
Fix a prompt $x$ and write $R(y):=R(x,y)$ for brevity. Let
\begin{equation*}
    P(y) := P^{\pitheta}(y\mid x),
    \qquad
    Q(y) := P^{\piroll}(y\mid x).
\end{equation*}
Define the conditional objective $J(\pi\mid x):=\E_{y\sim P^{\pi}(\cdot|x)}[R(y)]$. By definition, the conditional performance
difference is
\begin{equation*}
    J(\pitheta\mid x) - J(\piroll\mid x)
    =
    \sum_{y} \big(P(y)-Q(y)\big)\,R(y).
\end{equation*}
We use the telescoping product identity (cf.\ the proof of \Cref{lem:seq-tv-sum}): for any sequences $(a_t)_{t=1}^{T}$ and
$(b_t)_{t=1}^{T}$,
\begin{equation*}
    \prod_{t=1}^{T} a_t - \prod_{t=1}^{T} b_t
    =
    \sum_{t=1}^{T}\left(\prod_{k=1}^{t-1} b_k\right)(a_t-b_t)\left(\prod_{j=t+1}^{T} a_j\right).
\end{equation*}
Apply this identity with $a_t=\pitheta(y_t\mid c_t)$ and $b_t=\piroll(y_t\mid c_t)$ to obtain
\begin{align*}
    P(y)-Q(y)
    &=
    \sum_{t=1}^{T}
    \left(\prod_{k=1}^{t-1}\piroll(y_k\mid c_k)\right)
    \big(\pitheta(y_t\mid c_t)-\piroll(y_t\mid c_t)\big)
    \left(\prod_{j=t+1}^{T}\pitheta(y_j\mid c_j)\right).
\end{align*}
Factor out $Q(y)=\prod_{k=1}^{T}\piroll(y_k\mid c_k)$ and rewrite the difference in terms of per-token ratios
$\rho_t=\pitheta(y_t\mid c_t)/\piroll(y_t\mid c_t)$:
\begin{align*}
    P(y)-Q(y)
    &=
    Q(y)\sum_{t=1}^{T} (\rho_t-1)\left(\prod_{j=t+1}^{T}\rho_j\right).
\end{align*}
Substituting this into the performance difference yields
\begin{align*}
    J(\pitheta\mid x) - J(\piroll\mid x)
    &=
    \E_{y \sim Q}\!\left[
        R(y)\sum_{t=1}^{T} (\rho_t-1)\left(\prod_{j=t+1}^{T}\rho_j\right)
    \right].
\end{align*}
Finally, add and subtract the term with the future product set to $1$:
\begin{equation*}
    (\rho_t-1)\left(\prod_{j=t+1}^{T}\rho_j\right)
    =
    (\rho_t-1)
    -
    (\rho_t-1)\left(1-\prod_{j=t+1}^{T}\rho_j\right).
\end{equation*}
Summing over $t$ and taking expectations shows
\begin{equation*}
    J(\pitheta\mid x) - J(\piroll\mid x)
    =
    \E_{y \sim Q}\!\left[
        R(y)\cdot \sum_{t=1}^{T}(\rho_t-1)
    \right]
    -
    \E_{y \sim Q}\!\left[
        R(y)\cdot \sum_{t=1}^{T}(\rho_t-1)\left(1-\prod_{j=t+1}^{T}\rho_j\right)
    \right],
\end{equation*}
which is the conditional analogue of \Cref{def:llm-surrogate}. Averaging over $x\sim P(x)$ yields the stated identity for
$J(\pitheta)-J(\piroll)$.
\end{proof}

\subsection{Proof of the Remainder Bounds in \texorpdfstring{\Cref{rem:fh-tr-bound}}{Remark}}

\begin{proof}
We bound the remainder term from \Cref{def:llm-surrogate} using $|R(x,y)|\le R_{\max}$:
\begin{align*}
    \Delta^{\mathrm{LLM}}(\piroll,\pitheta)
    &\le
    \E_{x,y \sim P(x)P^{\piroll}(\cdot|x)}\!\left[
        |R(x,y)|\sum_{t=1}^{T}|\rho_t-1|\left|1-\prod_{j=t+1}^{T}\rho_j\right|
    \right]
    \\
    &\le
    R_{\max}\sum_{t=1}^{T}
    ...
    \right].
\end{align*}
Condition on the prefix $(x,y_{\le t})$ and denote by $P^{\pi}_{>t}(\cdot\mid c_{t+1})$ the distribution over suffixes
$y_{t+1:T}$ induced by policy $\pi$ starting from context $c_{t+1}=(x,y_{\le t})$. Then, for fixed $(x,y_{\le t})$,
\begin{align*}
    \E_{y_{t+1:T}\sim P^{\piroll}_{>t}(\cdot\mid c_{t+1})}\!\left[\left|1-\prod_{j=t+1}^{T}\rho_j\right|\right]
    &=
    2\,\DTV\big(P^{\piroll}_{>t}(\cdot\mid c_{t+1})\,\|\,P^{\pitheta}_{>t}(\cdot\mid c_{t+1})\big),
\end{align*}
which is the same identity as \Cref{lem:tv-ratio-identity} applied to the pair of suffix distributions. Therefore,
\begin{align*}
    \Delta^{\mathrm{LLM}}(\piroll,\pitheta)
    &\le
    2R_{\max}\sum_{t=1}^{T}
    \E_{x \sim P(x),\, y_{\le t} \sim P^{\piroll}(\cdot|x)}\!\left[
        |\rho_t-1|\cdot
        \DTV\big(P^{\piroll}_{>t}(\cdot\mid c_{t+1})\,\|\,P^{\pitheta}_{>t}(\cdot\mid c_{t+1})\big)
    \right].
\end{align*}

\textbf{Quadratic max-TV bound.} For any fixed $(x,y_{\le t})$, applying \Cref{lem:seq-tv-sum} to the length-$(T-t)$ suffix generation
problem gives
\begin{equation*}
    \DTV\big(P^{\piroll}_{>t}(\cdot\mid c_{t+1})\,\|\,P^{\pitheta}_{>t}(\cdot\mid c_{t+1})\big)
    \le
    \sum_{k=t+1}^{T}\Dtvtokmax
    =
    (T-t)\,\epsilon,
\end{equation*}
since $\epsilon=\Dtvtokmax$ upper bounds every token-level TV divergence. Substituting this bound yields
\begin{align*}
    \Delta^{\mathrm{LLM}}(\piroll,\pitheta)
    &\le
    2R_{\max}\sum_{t=1}^{T}(T-t)\,\epsilon\cdot \E_{c_t \sim d_t^{\piroll}}\!\left[
        \E_{y_t \sim \piroll(\cdot\mid c_t)}\!\big[|\rho_t-1|\big]
    \right]
    \\
    &=
    4R_{\max}\sum_{t=1}^{T}(T-t)\,\epsilon\cdot \E_{c_t \sim d_t^{\piroll}}\!\big[\Dtvtok(c_t;\piroll,\pitheta)\big]
    \qquad
    (\text{by \Cref{lem:tv-ratio-identity}})
    \\
    &\le
    4R_{\max}\sum_{t=1}^{T}(T-t)\,\epsilon\cdot \epsilon
    =
    2R_{\max}\,T(T-1)\,\epsilon^2.
\end{align*}

\textbf{Linear expected-sum-TV bound.} Alternatively, use the trivial bound $\DTV(\cdot\|\cdot)\le 1$ to obtain
\begin{align*}
    \Delta^{\mathrm{LLM}}(\piroll,\pitheta)
    &\le
    2R_{\max}\sum_{t=1}^{T}
    \E_{c_t \sim d_t^{\piroll}}\!\left[
        \E_{y_t \sim \piroll(\cdot\mid c_t)}\!\big[|\rho_t-1|\big]
    \right]
    \\
    &=
    4R_{\max}\sum_{t=1}^{T}\E_{c_t \sim d_t^{\piroll}}\!\big[\Dtvtok(c_t;\piroll,\pitheta)\big]
    =
...
\section{Gradient Form of the Implementation Surrogate}
\label[appsec]{app:grad-impl-surrogate}

This section provides a derivation of the gradient expression stated in \Cref{rem:grad-impl-surrogate}. The derivation is standard in
policy-gradient analyses; see, e.g., \citet{qi2026rethinkingtrustregionllm} for related surrogate-gradient comparisons in the LLM regime.

\subsection{Proof of \texorpdfstring{\Cref{rem:grad-impl-surrogate}}{Remark}}

\begin{proof}
Recall the implementation surrogate from \Cref{def:impl-surrogate}:
\begin{equation*}
    \tilde{\mathcal{L}}_{\piroll}(\pitheta)
    =
    \E_{\piroll}\!\left[A \cdot \sum_{t=1}^{T}\rho_t\right],
    \qquad
    \rho_t
    =
    \frac{\pitheta(y_t\mid c_t)}{\piroll(y_t\mid c_t)}.
\end{equation*}
During a policy update, the behavior policy $\piroll$ is fixed, so the expectation does not depend on $\theta$. Treating $A$ as a detached
scalar weight, differentiate under the expectation to obtain
\begin{equation*}
    \nabla_\theta \tilde{\mathcal{L}}_{\piroll}(\pitheta)
    =
    \E_{\piroll}\!\left[A \cdot \sum_{t=1}^{T}\nabla_\theta \rho_t\right].
\end{equation*}
Since $\piroll$ is independent of $\theta$, one has
\begin{equation*}
    \nabla_\theta \rho_t
    =
    \rho_t \nabla_\theta \log \pitheta(y_t\mid c_t).
\end{equation*}
...
\begin{equation*}
    ...
    =
    \E_{\piroll}\!\left[
        A \cdot \sum_{t=1}^{T} \rho_t\,\nabla_\theta \log \pitheta(y_t\mid c_t)
    \right],
\end{equation*}
which is the claimed score-function form.
\end{proof}
