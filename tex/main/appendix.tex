...
\paragraph{Backend discrepancies.}
Inference and training stacks often differ in attention kernels, precision, and parallel reduction order. Since floating-point addition is
non-associative, small numerical differences in normalization constants can compound autoregressively over long horizons.

\paragraph{MoE routing discontinuities.}
For Mixture-of-Experts models, the Top-$K$ routing operator is discontinuous. Small perturbations to router logits can change the selected
experts, producing large local deviations in token probabilities and hence large importance ratios $\rho_t$.

\paragraph{Distributed staleness.}
Actor--learner architectures generate rollouts with a lagging snapshot while the learner continues to update parameters, so $\piroll$ can
differ from $\pitheta$ even when inference and training implementations are otherwise identical.
...
Following~\citet{qi2026rethinkingtrustregionllm}, the trust-region anchor (and hence the importance ratio) must be tied to the original
rollout distribution that generated the data, which we denote by $\piroll$. Accordingly, we compute $\rho_t$ using rollout-time cached
log-probabilities under $\piroll$.

\paragraph{Notation mapping (PPO-style $\piold$).}
...
snapshot coincides with the data-generating policy $\piroll$ (so $\piold \equiv \piroll$ in our notation).
...
\paragraph{Variant E (soft, continuous prefix gating).}
\label{app:variant-e}
