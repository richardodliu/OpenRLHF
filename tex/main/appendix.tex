...
In practical training code, each sample typically concatenates prompt and response tokens and may include padding or packing. Our theory
therefore adopts the convention that $y=(y_1,\ldots,y_T)$ indexes only the response (action) tokens, and sums such as
$\sum_{t=1}^{T}(\cdot)$ are understood to range over these action positions. This matches the standard implementation behavior of
selecting response-token positions (e.g., via an action mask) while omitting prompt tokens from the policy-gradient objective.
...
\end{align}
Thus, if $t < (\log\rho_1+\varepsilon)/(\tau_+ + \varepsilon)$, then $\log\rhoprefix{t}>\tau_+$ and hence $M_t^{\mathrm{prefix}}=0$.
...
\begin{align}
    ...
    = \frac{\log\rho_1-\varepsilon}{t} + \varepsilon.
\end{align}
If $t < (-\log\rho_1+\varepsilon)/(\tau_-+\varepsilon)$, then $\log\rhoprefix{t} < -\tau_-$ and hence $M_t^{\mathrm{prefix}}=0$.
...
\paragraph{Variant B (absorbing prefix-average mask).}
...
\paragraph{Variant C (prefix-max mask / max-filtering).}
...
\paragraph{Variant D (prefix-score mask / bound-driven).}
...
\paragraph{Variant E (prefix average $k_3$ / KL-proxy).}

TRM~\citep{li2025trm} advocates the sample-based proxy
\begin{equation}
    k_3(\rho) := \rho - 1 - \log\rho,
\end{equation}
which satisfies $\E_{\piroll}[k_3(\rho_t)\mid c_t]=\DKL(\piroll(\cdot\mid c_t)\,\|\,\pitheta(\cdot\mid c_t))$ under the ratio definition
$\rho_t=\pitheta/\piroll$ and support overlap. This motivates a prefix-average KL-proxy mask:
\begin{equation}
    \overline{k}_{3,t}
    := \frac{1}{t}\sum_{s=1}^{t} k_3(\rho_s),
    \qquad
    M_t^{\mathrm{prefix\text{-}k3}} := \mathbb{I}\!\big[\overline{k}_{3,t} \le \delta_{k3}\big].
\end{equation}
Unlike ratio-interval thresholding, this variant targets a prefix-level \emph{average} KL proxy and can be interpreted as a causal
refinement of average-based trust-region criteria.
