...
\begin{itemize}
    \item \textbf{Token-level (ICEPOP):} $M_t^{\mathrm{token}} = \mathbb{I}[\rho_t \in [\lambda,\Lambda]] = \mathbb{I}[r_t \in [-\tau_-,\tau_+]]$.
    \item \textbf{Sequence-level (seq-mask-tis):} $M_t^{\mathrm{seq}} = \mathbb{I}[\bar{r} \in [-\tau_-,\tau_+]]$ for all $t$,
    where $\bar{r} := \frac{1}{P_T}\sum_{s=1}^{T}r_s m_s$ is the sequence-level average log-ratio.
    \item \textbf{Prefix cumulative:} $M_t^{\mathrm{prefix}} = \mathbb{I}[L_t/P_t \in [-\tau_-,\tau_+]]$, where
    $L_t = \sum_{s\le t}r_s m_s$ and $P_t=\sum_{s\le t}m_s$.
\end{itemize}

\begin{proof}[Proof of \Cref{thm:prefix-tighter}]
\textbf{Action-token indexing.} By \Cref{rem:action-indexing}, we may index only over active action-token positions and assume the
trajectory is unpadded. Equivalently, we take $m_t\equiv 1$, so $P_t=t$ and $L_t=\sum_{s=1}^{t}r_s$.

\textbf{Part (a).} Assume $\rho_1 \notin [\lambda,\Lambda]$ and $|r_t| \le \varepsilon$ for all $t>1$, with
...
\textbf{Part (b).} Assume there exists $t^*\in\{1,\ldots,T\}$ such that $\rho_{t^*}\notin[\lambda,\Lambda]$ and $|r_t|\le \varepsilon$
for all $t\neq t^*$, with
...
\textbf{Part (c).} If $r_s > \tau_+$ for all $s$, then for every $t$ the prefix average satisfies $L_t/P_t > \tau_+$, since it is an
