...
\begin{equation}
    ...
    \tau_- := -\log\lambda,
\end{equation}
so that $\rho_t \in [\lambda,\Lambda]$ is equivalent to $\log\rho_t \in [-\tau_-,\tau_+]$.
...
Assume $\rho_1 \notin [\lambda,\Lambda]$ and $|\log\rho_t| \le \varepsilon$ for all $t>1$, with
...
For the claimed regime where prefix rejects but sequence-level accepts, consider the pure single-spike case $\log\rho_{t^*}=L$ and
$\log\rho_t=0$ for $t\neq t^*$. Then $\log\rhoseq = L/T$ and $\log\rhoprefix{t^*} = L/t^*$. Since $t^* \le T$, we have $L/t^* \ge L/T$.
Thus whenever
\begin{equation}
    \frac{L}{t^*} > \tau_+
    \quad\text{and}\quad
