...
\begin{itemize}
    ...
    \end{equation}
    where $\bar{r}_t := \frac{1}{t}\sum_{s=1}^{t}r_s$.
\end{itemize}
...
\begin{equation}
    L_t := \sum_{s=1}^{t}r_s = r_1 + \sum_{s=2}^{t}r_s,
\end{equation}
so that $\bar{r}_t = L_t/t$. If $r_1>\tau_+$, then $r_s \ge -\varepsilon$ for all $s\ge2$, hence
\begin{equation}
    L_t \ge r_1 - \varepsilon(t-1).
\end{equation}
Therefore, if $t < (r_1+\varepsilon)/(\tau_+ + \varepsilon)$, then
\begin{align}
    \bar{r}_t
    = \frac{L_t}{t}
    &\ge \frac{r_1-\varepsilon(t-1)}{t}
...
$L_t \le r_1 + \varepsilon(t-1)$. If $t < (-r_1+\varepsilon)/(\tau_-+\varepsilon)$, then
\begin{align}
    \bar{r}_t
    = \frac{L_t}{t}
    &\le \frac{r_1+\varepsilon(t-1)}{t}
    = \frac{r_1-\varepsilon}{t} + \varepsilon
...
Since $\bar{r}_t$ is an average of values in $[-\tau_-,\tau_+]$, it follows that $\bar{r}_t \in [-\tau_-,\tau_+]$, and hence
...
for $t\neq t^*$. Then $\bar{r} = L/T$ and $\bar{r}_{t^*} = L/t^*$. Since $t^* \le T$, we have $L/t^* \ge L/T$.
Thus whenever
\begin{equation}
    \frac{L}{t^*} > \tau_+
    \quad\text{and}\quad
...
If $r_s > \tau_+$ for all $s$, then for every $t$ the prefix average satisfies $\bar{r}_t > \tau_+$, since it is an
