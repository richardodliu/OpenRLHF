...
\paragraph{Action-token indexing in implementations.}
In practical training code, a batch element often concatenates prompt and response tokens and may include padding or packing. Frameworks
therefore use an \emph{action mask} $m_t\in\{0,1\}$ to select the response (action) positions that contribute to the policy-gradient
objective. In this paper's theoretical development, we index the response as $y=(y_1,\ldots,y_T)$ and write sums as $\sum_{t=1}^{T}(\cdot)$,
which corresponds to the special case $m_t\equiv 1$ after restricting to action-token positions.
...
\begin{equation}
    ...
    \tau_- := -\log\lambda,
\end{equation}
so that $\rho_t \in [\lambda,\Lambda]$ is equivalent to $r_t \in [-\tau_-,\tau_+]$.
...
\begin{itemize}
    ...
    \begin{equation}
        M_t^{\mathrm{token}}
        := \mathbb{I}\!\big[\rho_t \in [\lambda,\Lambda]\big]
    ...
    \begin{equation}
        ...
        \qquad
        \bar{r} := \frac{1}{T}\sum_{s=1}^{T}r_s,
    \end{equation}
    for all $t$ (equivalently, $\rhoseq=\exp(\bar{r})$).
    \item \textbf{Prefix-level (Causal Trust Region proxy):}
    \begin{equation}
        M_t^{\mathrm{prefix}}
        := \mathbb{I}\!\big[\rhoprefix{t} \in [\lambda,\Lambda]\big]
...
\end{equation}
so $M_t^{\mathrm{token}}=1$ for all $t>1$, while $M_1^{\mathrm{token}}=0$ (since $\rho_1 \notin [\lambda,\Lambda]$). Hence token-level
ICEPOP masks only position $t=1$.

\smallskip
For the prefix mask, note first that $M_1^{\mathrm{prefix}}=0$ because $\rhoprefix{1}=\rho_1\notin[\lambda,\Lambda]$. To quantify how
long the rejection can persist, use \eqref{eq:prefix-is}:
\begin{equation}
    \log\rhoprefix{t}
    = \frac{1}{t}\sum_{s=1}^{t} r_s
    = \frac{1}{t}\left(r_1 + \sum_{s=2}^{t}r_s\right).
\end{equation}
If $r_1>\tau_+$, then $r_s \ge -\varepsilon$ for all $s\ge2$, hence $\sum_{s=2}^{t}r_s \ge -(t-1)\varepsilon$ and therefore
\begin{align}
    \log\rhoprefix{t}
    &\ge \frac{1}{t}\big(r_1-(t-1)\varepsilon\big)
    = \frac{r_1+\varepsilon}{t} - \varepsilon.
\end{align}
Thus, if $t < (r_1+\varepsilon)/(\tau_+ + \varepsilon)$, then $\log\rhoprefix{t}>\tau_+$ and hence $M_t^{\mathrm{prefix}}=0$.
The lower-violation case $r_1<-\tau_-$ is symmetric: since $r_s \le \varepsilon$ for $s\ge2$, we have
$\sum_{s=2}^{t}r_s \le \varepsilon(t-1)$ and therefore
\begin{align}
    \log\rhoprefix{t}
    &\le \frac{1}{t}\big(r_1+\varepsilon(t-1)\big)
    = \frac{r_1-\varepsilon}{t} + \varepsilon.
\end{align}
If $t < (-r_1+\varepsilon)/(\tau_-+\varepsilon)$, then $\log\rhoprefix{t} < -\tau_-$ and hence $M_t^{\mathrm{prefix}}=0$.
...
Since $\log\rhoprefix{t} = \frac{1}{t}\sum_{s=1}^{t}r_s$ is an average of values in $[-\tau_-,\tau_+]$, it follows that
$\log\rhoprefix{t} \in [-\tau_-,\tau_+]$, and hence $M_t^{\mathrm{prefix}}=1$ for all $t<t^*$. Therefore prefix masking can only occur
...
for $t\neq t^*$. Then $\bar{r} = L/T$ and $\log\rhoprefix{t^*} = L/t^*$. Since $t^* \le T$, we have $L/t^* \ge L/T$.
Thus whenever
\begin{equation}
    \frac{L}{t^*} > \tau_+
    \quad\text{and}\quad
...
If $r_s > \tau_+$ for all $s$, then for every $t$ the prefix average satisfies $\log\rhoprefix{t} > \tau_+$, since it is an
