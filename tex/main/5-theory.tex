...
behavior policy $\piroll$ and the training policy $\pitheta$. Prefix IS masks tokens whose prefix average log-ratio leaves the threshold
...
motivates viewing prefix masking as a sample-level proxy for trust-region style filtering on the same policy pair $(\piroll,\pitheta)$ that
...
$\rho_t=\pitheta/\piroll$ in \eqref{eq:ratio} during the subsequent gradient steps. This design follows the correct anchoring principle in
\citet{qi2026rethinkingtrustregionllm}; see \Cref{rem:three-to-two}.
...
$\piroll$. The prefix cumulative statistic $\mathrm{prefix\_is}(t)$ in \eqref{eq:prefix-is} aggregates this drift causally: it depends only
