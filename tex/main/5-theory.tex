...
\begin{equation}
    |\mathrm{Error}| \le \sum_{t=1}^T \underbrace{2\|g_t\|_\infty}_{\text{advantage factor}} \cdot \underbrace{\|d_t^{\pitheta} - d_t^{\piold}\|_{\mathrm{TV}}}_{\text{context shift factor}}.
\end{equation}
...
behavior policy $\piold$ and the training policy $\pitheta$. Prefix IS masks tokens whose prefix average log-ratio leaves the threshold
...
motivates viewing prefix masking as a sample-level proxy for trust-region style filtering on the same policy pair $(\piold,\pitheta)$ that
appears in the error decomposition.

\begin{remark}[Heuristic complementarity]
\label{rem:complementarity-heuristic}
...
advantage factor in \eqref{eq:error-decomp}, while REINFORCE Pro targets policy drift between $\piold$ and $\pitheta$ through sample-level
prefix filtering.
...
\Cref{prop:alpha-beta}). Component~2 (prefix IS masking) attenuates high-variance gradient contributions from ratio-outlier tokens
...
(REINFORCE Max, \Cref{sec:reinforce-max}) and prefix-causal ratio masking (REINFORCE Pro, \Cref{sec:reinforce-pro}) into a single
...
advantages with the RLOO baseline and adaptive normalization, then applies prefix cumulative IS masking during the loss computation. We
...
\begin{algorithmic}[1]
    ...
    \STATE \textbf{Rollout:} Generate $\{y^{(1)}, \ldots, y^{(n)}\} \sim \piold(\cdot|x)$; store $\log \piold(y_t|c_t)$
    ...
    \STATE \textbf{Forward Pass:} Compute $\log \pitheta(y_t|c_t)$; use cached $\log \piold(y_t|c_t)$ from rollout
    \STATE \textbf{Ratio:} $\rho_t \leftarrow \exp\!\big(\log \pitheta(y_t|c_t) - \log \piold(y_t|c_t)\big)$
    \STATE \textbf{Prefix IS:} $r_t \leftarrow \log \rho_t$
    \STATE \hspace{2.6em} Compute $\mathrm{prefix\_is}(t)$ as in Eq.~\eqref{eq:prefix-is}
    \STATE \hspace{2.6em} Compute $M_t^{\mathrm{prefix}}$ as in Eq.~\eqref{eq:prefix-mask}
...
\emph{Rollout (Line~1).} Samples are generated by a rollout engine (e.g., vLLM) running the behavior policy $\piold$. The per-token
log-probabilities $\log\piold(y_t|c_t)$ are stored alongside the generated tokens; these cached values anchor the importance ratio
$\rho_t=\pitheta/\piold$ in \eqref{eq:ratio} during the subsequent gradient steps.
...
\emph{Forward Pass (Line~7).} The training framework computes log-probabilities under the current policy $\pitheta$. The old-policy
log-probabilities $\log\piold(y_t|c_t)$ are reused from the rollout cache, as in standard PPO implementations.

\emph{Prefix IS Masking (Lines~9--11).} The per-token log-ratio $r_t=\log\rho_t$ captures local policy drift between $\pitheta$ and
$\piold$. The prefix cumulative statistic $\mathrm{prefix\_is}(t)$ in \eqref{eq:prefix-is} aggregates this drift causally: it depends only
on positions $s\le t$, respecting the autoregressive order (\Cref{def:prefix-is}). The corresponding binary mask
$M_t^{\mathrm{prefix}}$ in \eqref{eq:prefix-mask} filters out positions where the prefix average log-ratio leaves
$[\log\lambda,\log\Lambda]$, enforcing a per-position, sample-level constraint (\Cref{cor:per-position-trust}).

\emph{Loss Computation (Line~12).} The loss \eqref{eq:promax-loss} multiplies the PPO-style clipped surrogate by a detached prefix gate.
Concretely, $M_t^{\mathrm{prefix}}$ is a binary mask (detached) and $\mathrm{PPOClip}(\rho_t,\hat{A}_t)$ in \eqref{eq:ppo-clip} is the
only term carrying gradients through $\rho_t$. The negative sign ensures that minimizing $\mathcal{L}$ maximizes the surrogate objective.
...
\textbf{IS correction methods.} DAPO~\citep{yu2025dapo} combined with TIS clamps the per-token importance ratio to a fixed interval $[\lambda, \Lambda]$, retaining a biased but bounded correction at every position. DAPO combined with ICEPOP takes a harder approach: tokens whose ratio falls outside the interval are zeroed out entirely, which eliminates bias from extreme weights but discards potentially useful gradient signal. Both methods operate at the token level and treat each position independently. As shown in \Cref{prop:token-is-fails}, an early deviation with $\rho_1\notin[\lambda,\Lambda]$ is corrected only at position~1, while all subsequent positions---whose contexts are sampled from a mismatched distribution---remain uncorrected.
