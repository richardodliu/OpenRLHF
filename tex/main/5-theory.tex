...
\emph{Prefix Causal Trust Region Masking (Lines~9--11).} The per-token log-ratio $r_t=\log\rho_t$ captures local policy drift between $\pitheta$ and
...
to a fixed interval $[\lambda, \Lambda]$, retaining a biased but bounded weight at every position. ICEPOP instead masks out tokens whose
...
independently and therefore cannot correct the downstream context shift caused by an early deviation (\Cref{prop:token-is-fails}).
