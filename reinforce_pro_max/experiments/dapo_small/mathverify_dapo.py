...
def extract_answer(passage: str) -> str:
    # Restrict the existing extraction logic to the final DAPO Answer: section.
    matches = list(re.finditer(r"(?im)^\s*Answer\s*:\s*", passage))
    if matches:
        passage = passage[matches[-1].end():].strip()
    if "\\boxed" in passage:
        return extract_boxed_answer(passage)
    if matches and passage:
        return passage.splitlines()[0].strip()
    return None
...
def reward_func(queries, prompts, labels, **kwargs):
    ...
    for i, (query, label) in enumerate(zip(queries, labels)):
        ...
        model_solution = query[-200:]  # The longest answer in MATH-500 has 159 characters
        ...
        try:
            ...
            processed_ground_truths = list(set(processed_ground_truths))
        ...
        if i == 0 and result == False:
            print(f"solution: {model_answer}, label: {label}, result: {result}")
